...
MARQUE_CIBLE = "Shiseido (Simulé)"
PLATEFORME = "E-commerce"
SITE_SOURCE = "Books to Scrape"
CATEGORIE = "Cosmétiques de Luxe"
...
with open("shiseido_books_complet.csv", "w", newline="", encoding="utf-8") as fichier:
    writer = csv.writer(fichier)
    writer.writerow(colonnes)

    for page in range(1, NB_PAGES + 1):
        url = BASE_URL.format(page)
        print(f"Scraping {MARQUE_CIBLE} - Page {page}")
        ...
        for p in produits:
            try:
                ...
                writer.writerow([
                    nom, MARQUE_CIBLE, prix, CATEGORIE, PLATEFORME, 
                    SITE_SOURCE, url_prod, note, avis, dispo, page
                ])
...
driver.quit()
print(f"Terminé : shiseido_books_complet.csv est prêt.")